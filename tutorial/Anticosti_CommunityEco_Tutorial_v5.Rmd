...
We'll start by filtering out the "Questionable" tree status to answer our question
```{r}

# Filtering out "Questionable"
tree_filter <- tree[Condition != "Questionable"]

# Assign 1 when dead and 0 when alive in a new column 
tree_filter[, status := ifelse(Condition == "Dead", 1, 0)]

# Calculate proportion of dead species by cell
tree_filter[, prop_dead := mean(status), by = .(Species, Cell)]
...
tree_lineplot <- unique(tree_filter[, .(Cell, Species, prop_dead, sp_ind_by_cell)])

tree_lineplot[, scaled := (sp_ind_by_cell - mean(sp_ind_by_cell)) / sd(sp_ind_by_cell), by = Species]

ggplot(tree_lineplot[Species == "Balsam fir"],
       aes(x = scaled,
           y = prop_dead)) +
  geom_point() +
