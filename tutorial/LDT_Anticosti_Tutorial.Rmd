...
```{r, echo = FALSE, out.width = "50%", fig.align = "center", warning=FALSE, fig.cap = "Anticosti Island, Québec, shown as a blue pin on a map of Eastern Canada and Northeastern United States of America"}
...
#### 3. Did the hemlock looper preferentially attack certain tree species?

#### 4. Based 1-3, what management recomendations would you have for coping with hemlock looper outbreaks in Québec's forests?
...
4. #### American larch - *Larix laricina*
5. #### Aspen - *Populus* spp.
6. #### Pine - *Pinus* spp.
7. #### Balsam fir - *Abies balsamea*

#### The boxplot shows the various tree species on the y-axis, and the x-axis represents the number of individuals per plot. 

#### Each grey circle in the plot represents a different observation. Recall that for a boxplot, the line in the center of the box represents the median number of individuals corresponding to a given tree species. 
...
### Did the hemlock looper preferentially attack certain tree species?
...
We'll start by filtering out the "Questionable" tree status, as it is unclear if these have or have not been attacked by the hemlock looper. Then, we will calculate the total abundance of tree species by plot, which were denoted as either "dead" or "alive." With this, like we did before, we will be able to calculate the abundance of a given species relative to the abundance of all species within a plot.
```{r}
...
# Calculate number of individuals of each species for each plot
...
We then evaluate the count of a tree species' deaths relative to all species denoted as "dead," in all plots :
```{r}
# Calculate total count of "dead" individuals by Plot
tree_filter[Condition == "Dead",
            total_dead := length(Condition),
            by = .(Plot, Compartment, Block)]

# Calculate count of "dead" individuals of each species for each Plot
tree_filter[Condition == "Dead",
...
# Identify cells with only with 1 species found dead
#tree_filter1[, unique_dead := length(unique(Species)), by = .(Plot, Compartment, Block)]
#tree_filter1 <- tree_filter1[unique_dead != 1]
...
question("Do hemlock loopers preferentially attack a given tree species?",
  ...
  answer("**Yes**, hemlock loopers preferentially attack **black spruce trees** because there are proportionally less dead trees of this species relative to their overall abundance in the forest"),
