...
ggplot(tree_filter,
       aes(x = prop_ind_sp_cell,
           y = prop_dead)) +
  geom_point() +
  scale_y_continuous(c(0, 1, 0.25)) +
  #geom_abline(intercept = 0, slope =1) +
  geom_smooth(method = "glm") +
  facet_wrap(~Species,
             scales = "free_x")

ggplot(tree_filter[Species == "Balsam fir"],
       aes(x = prop_ind_sp_cell,
           y = prop_dead)) +
  geom_point() +
  scale_y_continuous(c(0, 1, 0.25)) +
...
model <- glm(cbind(total_dead_sp_cell, total_dead - total_dead_sp_cell) ~ 
               prop_ind_sp_cell,
             data = tree_filter,
             family = binomial(link = logit))

# Generalist question : do they go in cell with more species? (Maybe not)
#tree_filter[, sp_cell := length(unique(Species)), by = Cell]

# dead based on other species present?????
```
