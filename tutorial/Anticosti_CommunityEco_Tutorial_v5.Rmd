...
The hemlock looper is a moth pest that is found [all across Canada](http://www.glfc.forestry.ca/VLF/ForumPresentations/heberthemlocklooper_08e.pdf), and into the USA, as far south as the state of Georgia. Since 1910, it has wreaked havoc in Québec's forests. Between 1910 and 1975, these outbreaks caused an estimated [24 million cubic meters of forest to be lost](https://fr.wikipedia.org/wiki/Lambdina_fiscellaria) in the province. Since then, additional outbreaks have occurred across Québec in places like Bas-Saint-Laurent , Gaspésie , and Anticosti Island. 
...
```{r, echo=FALSE, out.width="50%", fig.align="center", fig.cap="Severely defoliated balsam fir forest, photograph by Dr. Luc Jobin"}
...
### With our tree dataset, this tutorial will walk us through answering the following three driving questions:
...
#### The two previous plots beg the following question: were hemlock loopers attacking Balsam firs (SAB) because they are more abundant, or because this is their preferred prey? In other words, is this a **generalist** or **specialist** pest? What does this mean?
...
#### This question asks us to consider any density-dependent relationships. We thus need to consider the **relative proportion** of a given tree species, as it relates to the relative proportion of trees that were denoted as "dead." (THE RELATIONSHIP IS NOT DENSITY BUT RELATIVE ABUNDANCE INSTEAD. dENSITY WOULD RELATE TO ABUNDANCE RELATIVE TO SIZE OF PLOT IN METERS)
...
### Let's investigate the density-dependent relationship

We'll start by filtering out the "Questionable" tree status to answer our question, and calculate the total abundance of species by cell, either dead or alive. With this, we'll be able to calculate the abundance of a focal specie relative to the abundance of all species within a cell.
```{r}
...
We then evaluate the proportion of dead tree species relative to all dead species within a cell:
```{r}
...
# Filter out Aspen and Pine because only two observations
species <- c("Balsam fir", "Black spruce", "White spruce")
tree_filter <- tree_filter[Species %in% species]

# Check to filter cells only with 1 species found dead
