...
##### → We would see this on our graph if a tree species' points lie **above the one-to-one line** (orange triangle), as this means that the hemlock looper kills more trees of a given species (tree denoted as "dead") even if the relative abundance of this tree species is lower.
...
We'll start by filtering out the "Questionable" tree status to answer our question, and calculate the total abundance of tree species by plot, either dead or alive. With this, we'll be able, like we did before, to calculate the abundance of a focal species relative to the abundance of all species within a plot.
```{r}
...
We then evaluate the count of a tree species' deaths relative to all dead species in all plots :
```{r}
...
# Calculate each species' deaths relative to other species
tree_filter[Condition == "Dead",
