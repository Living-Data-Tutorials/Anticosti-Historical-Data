...
# Change "Pine" to "Pine"
# Change "Aspen" to "Aspen"
tree$Species <- gsub("Larch", "American larch", tree$Species)
tree$Species <- gsub("Pine", "Pine", tree$Species)
tree$Species <- gsub("Aspen", "Aspen", tree$Species)
...
#### We thus need to consider the **relative proportion** of a given tree species, our x-axis or independent variable, as it relates to the relative proportion of trees that were denoted as "dead," our y-axis or dependent variable, which is proxy for predation pressure.   

<br> 

#### With this in mind, we can create the following schematic to predict three possible scenarios:

```{r, echo=FALSE, out.width="70%", fig.align="center", fig.cap="Three possible scenarios regarding the hemlock loopers' predation behavior"}
...
##### → We would see this on our graph as a **one-to-one relationship** (diagonal black line), where a tree species' abundance in the forest is directly proportional to the number of trees from that species denoted as "dead."

#### (2) The hemlock looper prefers to predate upon some species versus others -- it is a **specialist** pest species. 

##### → We would see this on our graph if a species' points lie **above the one-to-one line** (orange triangle), as this means that more individuals of a given tree species are denoted as "dead" relative to their relative abundance in the forest. 

#### (3) The hemlock looper avoids some species, and thus these are less predated upon -- it is a **specialist** pest species. 

##### → We would see this on our graph if a species' points lie **below the one-to-one line** (blue triangle), as this means that less individuals of a given tree species are denoted as "dead" relative to their relative abundance in the forest. 
